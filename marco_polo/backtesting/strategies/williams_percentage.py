...
class WilliamsPercentage(Strategy):
    portfolio_size = 5
    ...
    def add_tech_ind(self, price_map):
        ...
        for symbol in price_map:
            ...
            symbol_data['percentage'] = ta.WILLR(symbol_data['high'], symbol_data['low'], symbol_data['open'], timeperiod=14)
            new_price_map[symbol] = symbol_data

        return new_price_map

    def rank_stocks(self, daily_data):
        sorted_daily_data = daily_data.sort_values('percentage', ascending=True)
